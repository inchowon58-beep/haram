...
export default async function GuidePage({ params }: Props) {
  ...
  return (
    <article className="pb-8 pt-24">
      ...
      <div className="haram-container max-w-3xl pt-10">
      ...
      {/* ---------- 소개 / 미션 ---------- */}
      {missionSection && (
        <section className="haram-container max-w-3xl">
          ...
          <div className="haram-container">
            <details className="guide-services-fold group">
              ...
              <div className="guide-services-body">
                <div className="haram-card-list" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
                  {page.services.map((svc, i) => (
                    <article key={svc.title + i} className="haram-card">
                      <div className="haram-card-thumb">
                        ...
                        />
                        <span className="haram-card-badge">
      ...
      {/* ---------- 보호·시설 안내 ---------- */}
      {facilitySection && (
        <section className="guide-prose-section haram-container max-w-3xl">
          <h2 className="font-display text-2xl text-[var(--ink)] md:text-3xl">
            {facilitySection.h2}
          </h2>
          <div className="ornament" />
      ...
      {page.promises && page.promises.length > 0 && (
        <section className="haram-section">
          <div className="haram-container">
            <div className="haram-sec-header">
              <span className="haram-badge">Promise</span>
              <h2 className="haram-sec-title">
                {page.promisesTitle || `${SITE.brand}의 세 가지 약속`}
              </h2>
            </div>
            <div className="haram-promise-list">
              {page.promises.map((pr, i) => (
                <div key={pr.title + i} className="haram-promise-item">
                  <div className="haram-promise-icon">
                    <span style={{ fontWeight: 800, fontSize: "1.3rem" }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                  </div>
                  <p className="haram-promise-title">{pr.title}</p>
                  <p className="haram-promise-desc">{pr.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {page.processSteps && page.processSteps.length > 0 && (
        <section className="haram-section haram-section-alt">
          <div className="haram-container">
            <div className="haram-sec-header">
              <span className="haram-badge">Process</span>
              <h2 className="haram-sec-title">
      ...
      {otherSections.map((sec, si) => (
        <section key={sec.h2 + si} className="guide-prose-section haram-container max-w-3xl">
      ...
      {page.faqs?.length > 0 && (
        <section className="guide-prose-section haram-container max-w-3xl">
          ...
          <div className="haram-faq-list">
            {page.faqs.map((f, i) => (
              <details key={f.q + i} className="haram-faq-item group">
                <summary className="haram-faq-q [&::-webkit-details-marker]:hidden" style={{ listStyle: "none" }}>
                  <span>{f.q}</span>
                  <span className="haram-faq-icon shrink-0 transition-transform group-open:rotate-45">+</span>
                </summary>
                <p className="haram-faq-a">{f.a}</p>
              </details>
            ))}
          </div>
        </section>
      ...
      {/* ---------- CTA ---------- */}
      <div className="haram-container max-w-3xl guide-cta-wrap">
        <aside className="haram-contact-cta guide-contact-box flex flex-col items-center gap-5 text-center">
          <p className="haram-contact-eyebrow">Contact — 전국파양입소 · 무료분양</p>
